...
class StockConsumption(models.Model):
    ...
    def _compute_has_account_moves(self):
        for consumption in self:
            if consumption.state == 'done' and consumption.move_ids:
    ...
    state = fields.Selection(string='Status', default='draft',
                             selection=[('draft', 'Draft'), ('cancel', 'Cancelled'), ('confirm', 'In Progress'),
                                        ('done', 'Validated')], copy=False, index=True, readonly=True, tracking=True)
    ...
    product_ids = fields.Many2many('product.product', string='Products', check_company=True, required=True,
    ...
    location_ids = fields.Many2many('stock.location', string='Locations', index=True, readonly=True, check_company=True,
    ...
    def action_start(self):
        self.ensure_one()
        self._action_start()
        return self.action_view_consumptions_lines()
    ...
    def action_view_consumptions_lines(self):
        ...
        action = {
            ...
            'view_id': self.env.ref(
                'ts_pos_profit_margin_report.view_consumption_lines').id,
    ...
    def create_consumption_lines(self, vals):
        ...
        for location_id in vals['location_ids']:
            for product_id in self.env['product.product'].browse(vals['product_ids']):
                if product_id.is_recipe and product_id.recipe_structure_ids:
                    for recipe_product in product_id.recipe_structure_ids:
                        line = {
                            "consumption_id": self.id,
                            "product_id": recipe_product.product_id.id,
                            "location_id": recipe_product.location_id.id,
                            "quantity": recipe_product.product_id.qty_available,
                            "consume_qty": recipe_product.qty_to_consum,
                        }
                        lines.append(line)
                else:
                    line = {
    ...
    def action_validate(self):
        ...
        for line in self.line_ids:
            ...
            if float_compare(line.inventory_diff_quantity, 0,
                ...
                move_vals.append(
                    line._get_inventory_move_values(line.inventory_diff_quantity,
