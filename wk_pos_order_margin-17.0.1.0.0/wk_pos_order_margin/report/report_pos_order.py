...
class ReportPosOrder(models.Model):
    ...
    wk_margin = fields.Float('Margins')

    def _select(self):
        return super(ReportPosOrder, self)._select() + ", SUM(l.wk_margin) AS wk_margin"
...
class ReportSaleDetails(models.AbstractModel):
        ...
        for category_name, product_list in products_sold.items():
            category_dictionnary = {
                ...
                'products': sorted([{
                    ...
                    'wk_margin':round(qty[1],2),
                    'price_unit': price_unit,
                    'discount': discount,
                    'uom': product.uom_id.name
                } for (product, price_unit, discount,purchase_price), qty in product_list.items()], key=lambda l: l['product_name']),
            }
    ...
    def _get_products_and_taxes_dict(self, line, products, taxes, currency):
        key2 = (line.product_id, line.price_unit, line.discount,line.total_cost)
        ...
        for key1 in keys1:
            ...
            products[key1][key2][1] += line.wk_margin            
    ...
    def _get_total_and_qty_per_category(self, categories):
        ...
        for category_dict in categories:
            ...
            for product in category_dict['products']:
                ...
                cost_price_cat+=product['cost_price']
                wk_margin_cat+=product['wk_margin']
            category_dict['total'] = total_cat
            category_dict['qty'] = qty_cat
            category_dict['cost_price'] = cost_price_cat
            category_dict['wk_margin'] = wk_margin_cat
        ...
        total_cost =sum ([product['cost_price'] for product in unique_products])
        total_margin= sum([product['wk_margin'] for product in unique_products])

        return categories, {'total': all_total, 'qty': all_qty, 'total_cost':total_cost ,'total_margin':total_margin}