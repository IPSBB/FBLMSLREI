...
class ProductProduct(models.Model):
    ...
    def _compute_reorder_qty_min(self):
        for product in self:
            if product.sudo().orderpoint_ids:
                for i in product.sudo().orderpoint_ids[0]:
...
class ProductTemplate(models.Model):
    ...
    def _compute_reorder_qty_min(self):
        for product in self:
            if product.product_variant_id.sudo().orderpoint_ids:
                for i in product.product_variant_id.sudo().orderpoint_ids[0]:
                    qty = i.product_min_qty
