...
class PosSessionInherit(models.Model):
	...
	def get_closing_control_data(self):
		self.ensure_one()
		order = self._get_closed_orders().filtered(lambda o: o.state == 'paid' or o.state == 'invoiced')
		orders = order + self._get_closed_orders().filtered(lambda o: o.is_partial == True)
		...
		total_default_cash_payment_amount = sum(payments.filtered(
			lambda p: p.session_id.id in self.ids and p.payment_method_id == default_cash_payment_method_id).mapped(
		...
		closing_vals = {
			...
			'other_payment_methods': [{
				'name': pm.name,
				'amount': sum(orders.payment_ids.filtered(
					lambda p: p.session_id.id in self.ids and p.payment_method_id == pm).mapped('amount')),
	...
	def _accumulate_amounts(self, data):
		res = super(PosSessionInherit, self)._accumulate_amounts(data)

		closedOrders = self._get_closed_orders()
		for order in closedOrders:
			for payment in order.payment_ids:
				for payment_method in payment:
					if payment_method.payment_method_id.journal_id:
						if payment_method.payment_method_id.journal_id.is_credit:
							...
							closed_orders = self._get_closed_orders().filtered(lambda order: order.is_partial == False)
										...
										if order_is_invoiced:
											if is_split_payment:
												split_inv_payment_receivable_lines[
													payment] |= payment.account_move_id.line_ids.filtered(
													lambda line: line.account_id == pos_receivable_account)
												split_invoice_receivables[payment] = self._update_amounts(
													split_invoice_receivables[payment], {'amount': payment.amount},
													order.date_order)
											else:
								...
								if not order_is_invoiced:
									...
									for order_line in order.lines:
										...
										sale_key = (
											...
											# for taxes
											tuple((tax['id'], tax['account_id'], tax['tax_repartition_line_id']) for tax
												  in line['taxes']),
											line['base_tags'],
										)
										sales[sale_key] = self._update_amounts(sales[sale_key],
																			   {'amount': line['amount']},
																			   line['date_order'], round=False)
									...
									if self.company_id.anglo_saxon_accounting and order.picking_ids.ids:
										...
										for move in stock_moves:
											...
											amount = signed_product_qty * move.product_id._compute_average_price(0,
																												 move.quantity,
																												 move)
									...
									if self.config_id.cash_rounding:
										...
										rounding_difference = self._update_amounts(rounding_difference,
																				   {'amount': diff}, order.date_order)
